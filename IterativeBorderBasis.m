function [Basis,C_0] = IterativeBorderBasis(Polynomials,Variables,R,Degree,tol)
...
Monomials_of_degree_N = Construct_Monomials(Degree,Variables,zeros(Variables,1)');
...
C_n = Intersection_Spaces(Prolongation_times(C_0,Monomials_of_degree_N,Variables),R,tol);
while Same_Spaces(C_0,C_n,tol) == 0      
     C_0 = C_n;
     C_n = Intersection_Spaces(Prolongation_times(C_0,Monomials_of_degree_N,Variables),R,tol); %problem due to not zero elements is the intersection not always right.
end
...
B = Construct_Basis(B,[Coefficients({{[1],[zeros(Variables,1)']}},Monomials_of_degree_N)],C_0,R,Variables,Monomials_of_degree_N,tol);
...
for i = 1:Cardinality_of_R(2)
    ...
    [Basis,C_0] = IterativeBorderBasis(Polynomials,Variables,Prolongation_plus(R,Monomials_of_degree_larger_N,Variables),Degree + 1,tol);
