function [Basis,C_0] = BorderBasis_standard(Polynomials,variables,tol)
...
Number_of_monomials = size(Construct_Monomials_standard(N + 1,variables,zeros(variables,1)')); % number of monomials of degree at most N
...
R = [];
Monomials_of_degree_N = Construct_Monomials_standard(N + 1,variables,zeros(variables,1)'); % we have to consider N+1 since we will also multiply some polynomials by x_i and x_i^{-1}
...
[Basis,C_0] = IterativeBorderBasis_standard(Polynomials,variables,R,N+1,tol);
